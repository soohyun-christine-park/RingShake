...
public class RingshakeSelectActivity     
...
//** (서버사용)프라이버시 제거-	private static final int CMD_PRIVACY = 2;     
...
//	private static final int CMD_SET_AS_CONTACT = 7;       
	public RingshakeSelectActivity() {     
	}      
...
//** (서버사용)프라이버시 제거-			item = menu.add(0, CMD_PRIVACY, 0, R.string.menu_privacy);   
...
//** (서버사용)프라이버시 제거-			menu.findItem(CMD_PRIVACY).setVisible(true); 
		...
		public boolean onOptionsItemSelected(MenuItem item) {     
			...
			case CMD_ABOUT:             
				...
				return true;        
//** (서버사용)프라이버시 제거-			case CMD_PRIVACY:         
//				showPrivacyDialog();         
//				return true;       
			case CMD_SHOW_ALL:        
				mShowAll = true;          
...
//				menu.add(0, CMD_SET_AS_CONTACT, 0, R.string.context_menu_contact);       
		...
		private void confirmDelete() {          
			...
			new AlertDialog.Builder(RingshakeSelectActivity.this)          
				.setTitle(title)          
				.setMessage(message)         
				.setPositiveButton(                
		...
		private void startRingshakeEditor() {        
			Cursor c = mAdapter.getCursor();        
			int dataIndex = c.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA);     
			String filename = c.getString(dataIndex);     
			try {           
